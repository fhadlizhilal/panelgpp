      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <select name="nametag" class="form-control" style="font-size: 12px;">
            <?php if($get_penilaianTmp['nametag'] == "-"){ ?>
              <option value="-">-</option>
            <?php }else{ ?>
              <option value="Ya">Ya</option>
              <option value="Lupa/Tidak" <?php if($get_penilaianTmp['nametag'] == "Lupa/Tidak"){ echo "selected" ;} ?>>Lupa/Tidak</option>
            <?php } ?>
          </select>
          <br>
          <input type="hidden" name="id" value="<?php echo $id; ?>">
          <center><input type="submit" class="btn btn-info" name="edit_nametag" value="Ubah"></center>
        </div>
      </div>
    </form>
  </div>
