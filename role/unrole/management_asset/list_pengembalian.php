...
<!-- Content Wrapper. Contains page content -->
              ...
              <div class="card-body">
                <table id="setHariLibur1" class="table table-bordered table-striped table-sm" style="font-size: 11px;">
                  <thead>
                    <tr>
                      <th width="1%">No</th>
                      <th width="13%">No Pengembalian</th>
                      <th width="8%">Entitas</th>
                      <th width="">Penanggungjawab</th>
                      <th width="">Project</th>
                  ...
                  <tbody>
                    <?php
                      $no = 1;
                      $q_get_pengembalian = mysqli_query($conn,"SELECT * FROM asset_pengembalian WHERE penanggungjawab = '$_SESSION[nik]' AND status = 'waiting for approval'");
                    ...
                    ?>
                      <tr>
                        <td><?php echo $no; ?></td>
                        <td>
                          <a href="#modal" data-toggle='modal' data-target='#show_detail_pengembalian_saya' data-id='<?php echo $get_pengembalian['id']; ?>' data-toggle="tooltip" data-placement="bottom" title="Detail Peminjaman">
                            <?php echo "RTN".$get_pengembalian['id']."/MA/".date("d-m-Y", strtotime($get_pengembalian['tanggal'])); ?>
                            </a>
                        </td>
                        ...
                        <td><?php echo date("d F Y", strtotime($get_pengembalian['tanggal'])); ?></td>
                        <td><span class="badge badge-warning"><?php echo $get_pengembalian['status']; ?></span></td>
                      </tr>
                    <?php $no++; } ?>
                  </tbody>
                </table>
              ...
              <div class="card-body">
                <table id="example3" class="table table-bordered table-striped table-sm" style="font-size: 12px;">
                  ...
                  <tbody>
                    <?php
                      $no=1;
                      $q_get_peminjaman = mysqli_query($conn, "SELECT * FROM asset_peminjaman WHERE peminjam = '$_SESSION[nik]' AND kd_project IS NULL AND id NOT IN (SELECT peminjaman_id FROM asset_pengembalian_selesai WHERE peminjaman_id IS NOT NULL) ORDER BY id DESC");
                      ...
                      <tr>
                        ...
                        <td>
                          <?php if($cek_pengembalian < 1){ ?>
                            <span class="badge badge-secondary">belum ada pengembalian</span>
                          <?php }else{ ?>
                            <span class="badge badge-info">sudah ada pengembalian</span>
                          <?php } ?>
                        </td>
                      </tr>
                    <?php $no++; } ?>
  ...
  <!-- Modal start here -->
  <div class="modal fade" id="show_detail_pengembalian_saya" role="dialog">
