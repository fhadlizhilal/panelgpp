<?php
  ...
  if(isset($_POST['update_peminjaman'])){
    if($_POST['update_peminjaman'] == "update"){
      //update peminjaman
      $update_peminjaman = mysqli_query($conn, "UPDATE asset_peminjaman SET tgl_pinjam = '$tgl_jam', kd_project = '$_POST[kode_project]', keterangan = '$_POST[keterangan_pinjam]' WHERE id = '$_POST[id]'");

      //DELETE peminjaman detail
      $delete_peminjaman_detail = mysqli_query($conn, "DELETE FROM asset_peminjaman_detail WHERE peminjaman_id = '$_POST[id]'");

      //Push data peminjaman detail baru
      $q_get_db_general = mysqli_query($conn, "SELECT * FROM asset_db_general WHERE jenis = '$_POST[jenis_form]'");
      while($get_db_general = mysqli_fetch_array($q_get_db_general)){
        ...
        if($_POST[$post_qty] > 0){
            $push_draft_detail = mysqli_query($conn, "INSERT INTO asset_peminjaman_detail VALUES('','$_POST[id]','$get_db_general[general_code]','$_POST[$post_qty]','$_POST[$post_keterangan]')");
          }
      }

      $_SESSION['alert_success'] = "Data Peminjaman Berhasil Diupdate!";

    }elseif($_POST['update_peminjaman'] == "submit"){
      //update peminjaman
      $update_peminjaman = mysqli_query($conn, "UPDATE asset_peminjaman SET tgl_pinjam = '$tgl_jam', kd_project = '$_POST[kode_project]', keterangan = '$_POST[keterangan_pinjam]', status = 'waiting for MA' WHERE id = '$_POST[id]'");
      ...
      $_SESSION['alert_success'] = "Data Peminjaman Berhasil Disubmit!";
    }
  }

  if(isset($_POST['batalkan_by_user'])){
    if($_POST['batalkan_by_user'] == "batalkan"){
      $batalkan_by_user = mysqli_query($conn, "UPDATE asset_peminjaman SET status = 'canceled by user' WHERE id = '$_POST[id]'");

      if($batalkan_by_user){
        $_SESSION['alert_success'] = "Peminjaman Berhasil Dibatalkan!";
      }else{
        $_SESSION['alert_danger'] = "Peminjaman Gagal Dibatalkan!";
      }
    }
  }
?>
              ...
              <div class="card-body">
                <table id="setHariLibur1" class="table table-bordered table-striped table-sm" style="font-size: 12px;">
                  ...
                  <tbody>
                    <?php
                      $no = 1;
                      $q_get_peminjaman = mysqli_query($conn, "SELECT * FROM asset_peminjaman WHERE peminjam = '$_SESSION[nik]' AND status = 'canceled by user' OR status = 'rejected by MA' OR status = 'completed' ORDER BY tgl_pinjam DESC");
