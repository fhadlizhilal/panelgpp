<?php
  ...
  if(isset($_POST['backto_onprogress'])){
    if($_POST['backto_onprogress'] == "to_onprogress"){

      $to_progress = mysqli_query($conn, "UPDATE asset_peminjaman SET status = 'on progress by MA' WHERE id = '$_POST[peminjaman_id]'");

        if($to_progress){
          $_SESSION['alert_success'] = "Berhasil! Peminjaman berhasil dikembalikan ke on progress!<br>Tunggu sampai halaman selesai diarahkan";
          echo "<meta http-equiv='refresh' content='0;url=index.php?pages=peminjamancompleted'>";
        }else{
          $_SESSION['alert_error'] = "Gagal! Peminjaman Gagal dikembalikan!";
        }
                ...
                <?php if($_SESSION['role'] == "management_asset"){ ?>
                  <div style="text-align: center; margin-top: 20px;">
                    <form method="POST" action="">
                      ...
                      <button type="submit" class="btn btn-warning btn-sm" onclick="return confirm('Yakin kembalikan peminjaman ini menjadi on progress?');" name="backto_onprogress" value="to_onprogress"><span class="fa fa-check"> Kembalikan ke On Progress</span></button>
                    </form>
                  </div>
                <?php } ?>
