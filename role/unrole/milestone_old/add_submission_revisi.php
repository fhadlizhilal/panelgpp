<?php
  ...
  $get_submissionRevisi = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM submission_list WHERE milestone_list_revisi = '$_GET[revid]'"));
  $get_person = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM karyawan WHERE nik = '$get_milestoneList[person]'"));

  $get_milestoneRevisi = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM milestone_list_revisi WHERE id = '$_GET[revid]'"));
  ...
  if(isset($_POST['add_attachment_images_submissionrevisi'])){
    if($_POST['add_attachment_images_submissionrevisi'] == "Submit"){
      if($_FILES["file"]["name"] != ""){
        ...
        if(!empty($_FILES["file"]["name"])) { 
            ...
            if(in_array($fileType, $allowTypes)){ 
                ...
                if($compressedImage){
                  $push_lampiran_images = mysqli_query($conn, "INSERT INTO submission_list_revisi_lampiranimg VALUES('','$_POST[revisi_id]','$fileName','$tanggal_jam')");
  ...
  if(isset($_POST['delete_lampiran_img_submissionrevisi'])){
    if($_POST['delete_lampiran_img_submissionrevisi'] == "delete"){
      $delete_lampiran_img_submissionrevisi = mysqli_query($conn, "DELETE FROM submission_list_revisi_lampiranimg WHERE id = '$_POST[id]'");
      if($delete_lampiran_img_submissionrevisi){
        unlink("../unrole/milestone/lampiran_images/".$_POST['nama_img']);
      }
    }
  }

  if(isset($_POST['add_attachment_file_submissionrevisi'])){
    if($_POST['add_attachment_file_submissionrevisi'] == "Submit"){
      ...
      if ($uploadOk == 1) {
          if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
              $push_lampiran_file = mysqli_query($conn, "INSERT INTO submission_list_revisi_lampiranfile VALUES('','$_POST[revisi_id]','$fileName','$tanggal_jam')");
  ...
  if(isset($_POST['delete_lampiran_file_submissionrevisi'])){
    if($_POST['delete_lampiran_file_submissionrevisi'] == "delete"){
      $delete_lampiran_file_submissionrevisi = mysqli_query($conn, "DELETE FROM submission_list_revisi_lampiranfile WHERE id = '$_POST[id]'");
      if($delete_lampiran_file_submissionrevisi){
        unlink("../unrole/milestone/lampiran_file/".$_POST['nama_file']);
      }
    }
  }

  if(isset($_POST['submit_submissionrevisi'])){
    if($_POST['submit_submissionrevisi'] == "Submit Submission"){
      $delete_submission_old = mysqli_query($conn, "DELETE FROM submission_list_revisi WHERE milestone_revisi_id = '$_POST[revisi_id]'");
      $push_submission = mysqli_query($conn, "INSERT INTO submission_list_revisi VALUES('','$_POST[revisi_id]','$_POST[comments]','$tanggal_sekarang')");
              ...
              <div class="card-body">
                <table class="table table-sm" style="font-size: 12px">
                  <tr>
                    ...
                    <td width="1%">:</td>
                    <td><?php echo $get_milestoneRevisi['comments']; ?></td>
                  </tr>
                  <tr>
                    <td>Person</td>
                    <td width="1%">:</td>
                  ...
                  <tr>
                    <td>Due Date</td>
                    <td width="1%">:</td>
                    <td><?php echo date("d F Y", strtotime($get_milestoneRevisi['due_date'])); ?></td>
                  </tr>
                </table>
                <br>
...
<!-- --------------------------------------------------- My Submission ------------------------------ -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div style="font-size: 12px; margin-bottom: 5px;">
                  <a href="#modal" data-toggle='modal' data-target='#show_add_attachmentimg_submissionrevisi' data-id='<?php echo $get_milestoneList['id']; ?>' data-toggle="tooltip" data-placement="bottom" title="Add Attachment Images">
                ...
                <div class="card-body p-0">
                  <div class="row">
                    <?php
                      $q_get_lampiran_images = mysqli_query($conn, "SELECT * FROM submission_list_revisi_lampiranimg WHERE milestone_revisi_id = '$_GET[revid]'");
                      while($get_lampiran_images = mysqli_fetch_array($q_get_lampiran_images)){
                    ?>
                      <div class="col-lg-4 col-2" style="margin-bottom: 10px;">
                        <img src="../unrole/milestone/lampiran_images/<?php echo $get_lampiran_images["nama_img"]; ?>" alt="..." style="width: 100%">
                        ...
                        <form id="myForm<?php echo $no; ?>" method="POST" action="" style="text-align: center;">
                          ...
                          <button type="submit" class="btn btn-danger btn-xs" name="delete_lampiran_img_submissionrevisi" value="delete" onclick="return confirm('Yakin delete gambar ini?')">
                ...
                <div style="font-size: 12px;">
                  <a href="#modal" data-toggle='modal' data-target='#show_add_attachmentfile_submissionrevisi' data-id='<?php echo $get_milestoneList['id']; ?>' data-toggle="tooltip" data-placement="bottom" title="Add Attachment File">
                ...
                <div class="card-body p-0">
                  <table class="table table-sm" style="font-size: 12px;">
                    ...
                    <tbody>
                      <?php
                        $no=1;
                        $q_get_lampiran_file = mysqli_query($conn, "SELECT * FROM submission_list_revisi_lampiranfile WHERE milestone_revisi_id = '$_GET[revid]'");
                        while($get_lampiran_file = mysqli_fetch_array($q_get_lampiran_file)){
                      ?>
                        <tr>
                          <td><?php echo $no; ?></td>
                          ...
                          <td style="font-size: 14px;">
                            <form id="myForm<?php echo $no; ?>" method="POST" action="" enctype="multipart/form-data">
                              ...
                              <button type="submit" class="btn btn-danger btn-xs" name="delete_lampiran_file_submissionrevisi" value="delete" onclick="return confirm('Yakin delete file ini?')">
                ...
                <form method="POST" action="">
                  <div class="form-group row" style="margin-bottom: 8px;">
                    <div class="col-sm-12">
                      ...
                      <textarea class="form-control form-control-sm" name="comments" required><?php echo $get_submissionRevisi['comments']; ?></textarea>
                    </div>
                  </div>
                  <center>
                    <input type="hidden" name="revisi_id" value="<?php echo $_GET['revid']; ?>">
                    <input type="hidden" name="milestone_id" value="<?php echo $_GET['id'] ?>">
                    <input type="submit" class="btn btn-success btn-flat" name="submit_submissionrevisi" value="Submit Submission">
                  </center>
                </form>
  ...
  <!-- Modal start here -->
  <div class="modal fade" id="show_add_attachmentimg_submissionrevisi" role="dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="myForm" method="POST" action="" enctype="multipart/form-data">
            ...
            <br>
            <input type="hidden" name="revisi_id" value="<?php echo $_GET['revid']; ?>">
            <input type="submit" class="btn btn-success" name="add_attachment_images_submissionrevisi" value="Submit">
          </form>
        </div>
      </div>
      <!-- /.modal-content -->
  ...
  <!-- Modal start here -->
  <div class="modal fade" id="show_add_attachmentfile_submissionrevisi" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <form id="myFormA" method="POST" action="" enctype="multipart/form-data">
            ...
            <br>
            <input type="hidden" name="revisi_id" value="<?php echo $_GET['revid']; ?>">
            <input type="submit" class="btn btn-success" name="add_attachment_file_submissionrevisi" value="Submit">
          </form>
        </div>
      </div>
      <!-- /.modal-content -->
