<?php
  ...
  //------------------------------------------ PEMINJAMAN TOOLS START -----------------------------------------
      //Simpan peminjaman tools
      if(isset($_POST['simpan_peminjaman_tools'])){
        if($_POST['simpan_peminjaman_tools'] == "Simpan"){
          //push info peminjaman
          mysqli_query($conn, "DELETE FROM tools_peminjaman_info WHERE nik = '$_SESSION[nik]'");
          mysqli_query($conn, "INSERT INTO tools_peminjaman_info VALUES('','$_SESSION[nik]','$_POST[kd_project]','$_POST[deskripsi]')");

          mysqli_query($conn, "DELETE FROM tools_peminjaman_tmp WHERE nik = '$_SESSION[nik]'");
          for($a=1;$a<=$_POST['data_max'];$a++){
            if($_POST["qty_".$a] > 0){
              $id_tools = $_POST["id_tools_".$a];
              $qty_tools = $_POST["qty_".$a];
              $catatan_tools = $_POST["catatan_".$a];
              $push_tmp = mysqli_query($conn, "INSERT INTO tools_peminjaman_tmp VALUES('','$_SESSION[nik]','$id_tools','$qty_tools','$catatan_tools')");
            }
          }
            $_SESSION['alert_success'] = "Berhasil! Draft peminjaman berhasil disimpan";
        }
      }

      //Submit peminjaman tools
      if(isset($_POST['submit_peminjaman_tools'])){
        if($_POST['submit_peminjaman_tools'] == "Ajukan"){
          $count_pinjam_tools = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tools_peminjaman"));
          $nomor_pinjam = $count_pinjam_tools + 1;
          if($nomor_pinjam < 10){
            $nomor_pinjam = "00".$nomor_pinjam;
          }elseif($nomor_pinjam < 100){
            $nomor_pinjam = "0".$nomor_pinjam;
          }
          
          //push info peminjaman
          $kd_pinjam = $nomor_pinjam."/MA/TLS/".date("m")."/".date("Y")."-A";
          $_SESSION['alert_success'] = $kd_pinjam;
          $push_peminjaman = mysqli_query($conn, "INSERT INTO tools_peminjaman VALUES('','$kd_pinjam','$_SESSION[nik]','$_POST[kd_project]','$_POST[deskripsi]','$this_date','','','','','','','Diajukan')");

          //push peminjaman detail
          for($a=1;$a<=$_POST['data_max'];$a++){
            if($_POST["qty_".$a] > 0){
              $id_tools = $_POST["id_tools_".$a];
              $qty_tools = $_POST["qty_".$a];
              $catatan_tools = $_POST["catatan_".$a];
              $push_tmp = mysqli_query($conn, "INSERT INTO tools_peminjaman_detail VALUES('','$kd_pinjam','$id_tools','$qty_tools','$catatan_tools')");
            }
          }

          //delete from table tmp & info
          mysqli_query($conn, "DELETE FROM tools_peminjaman_info WHERE nik = '$_SESSION[nik]'");
          mysqli_query($conn, "DELETE FROM tools_peminjaman_tmp WHERE nik = '$_SESSION[nik]'");

          if($push_peminjaman){
            $_SESSION['alert_success'] = "Peminjaman : ".$kd_pinjam." Berhasil Diajukan! <br><small><a href='index.php?pages=list-peminjaman'>Lihat List Peminjaman Saya</a></small>";
          }else{
            $_SESSION['alert_error'] = "Peminjaman : ".$kd_pinjam." Gagal Diajukan! <br><small>".mysqli_error($conn)."</small";
          }
        }
      }

      //Batalkan Peminjaman Tools
      if(isset($_POST['batalkan_tools'])){
        if($_POST['batalkan_tools'] == "Batalkan"){
          //edit status -> batal
          $batalkan_peminjaman = mysqli_query($conn, "UPDATE tools_peminjaman SET dibatalkan = '$this_date', status_pinjam = 'Dibatalkan' WHERE no_pinjam = '$_POST[no_pinjam]'");

          if($batalkan_peminjaman){
            $_SESSION['alert_success'] = "Peminjaman : ".$_POST['no_pinjam']." Berhasil Dibatalkan!";
          }else{
            $_SESSION['alert_error'] = "Peminjaman : ".$_POST['no_pinjam']." Gagal Dibatalkan! <br><small>".mysqli_error($conn)."</small";
          }
        }
      }

      //Edit peminjaman tools
      if(isset($_POST['edit_peminjaman_tools'])){
        if($_POST['edit_peminjaman_tools'] == "Ubah Peminjaman"){
          //ubah tools peminjaman
          $ubah_peminjaman_tools = mysqli_query($conn, "UPDATE tools_peminjaman SET kd_project = '$_POST[kd_project]', deskripsi_pinjam = '$_POST[deskripsi]', diajukan = '$this_date' WHERE no_pinjam = '$_POST[no_pinjam]'");

          //ubah list tools yang dipinjam
          mysqli_query($conn, "DELETE FROM tools_peminjaman_detail WHERE no_pinjam = '$_POST[no_pinjam]'");
          for($a=1;$a<=$_POST['data_max'];$a++){
            if($_POST["qty_".$a] > 0){
              $id_tools = $_POST["id_tools_".$a];
              $qty_tools = $_POST["qty_".$a];
              $catatan_tools = $_POST["catatan_".$a];
              $push_tmp = mysqli_query($conn, "INSERT INTO tools_peminjaman_detail VALUES('','$_POST[no_pinjam]','$id_tools','$qty_tools','$catatan_tools')");
            }
          }

          if($ubah_peminjaman_tools){
            $_SESSION['alert_success'] = "Peminjaman : ".$_POST['no_pinjam']." Berhasil Diubah!";
          }else{
            $_SESSION['alert_error'] = "Peminjaman : ".$_POST['no_pinjam']." Gagal Diubah! <br><small>".mysqli_error($conn)."</small";
          }

        }
      }

  //------------------------------------------ PEMINJAMAN TOOLS END -----------------------------------------
?>
...
<div class="wrapper">
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item <?php if($_GET['pages']=='dailyreport'){ echo "menu-open"; } ?>">
  ...
  <?php
    ...
    }elseif($_GET["pages"]=="list-peminjaman"){
      require_once "../peminjaman/list_peminjaman.php";
    }elseif($_GET["pages"]=="add-peminjaman"){
      require_once "../peminjaman/add_peminjaman.php";
