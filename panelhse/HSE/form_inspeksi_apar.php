...
<?php
  ...
  if(isset($_POST['add_dokumentasi_inspeksiapar_v2'])){
    if($_POST['add_dokumentasi_inspeksiapar_v2'] == "Upload"){
      $jml_uploaddokumentasi_berhasil = 0;
      $jml_uploaddokumentasi_gagal = 0;
      $nodate = date('YmdHis');
      $inspeksi_id = $_POST['inspeksi_id'];
      $target_dir = "../../role/HSE/foto_inspeksi_apar/";
      ...
      for ($i = 0; $i < $total; $i++) {
        ...
        if ($tmpFilePath != "" && in_array($imageFileType, $allowed_types)) {
            if($compressedImage){
                $push_dokumentasi_inspeksiborac = mysqli_query($conn, "INSERT INTO hse_inspeksilist_fotoapar VALUES('','$inspeksi_id','$filename','$keterangan[$i]')");
                    ...
                    <td align="center" colspan="3">
                        <b>DOKUMENTASI</b><br>
                          <form id="myForm4" action="" method="post" enctype="multipart/form-data">
                            ...
                            <input type="submit" class="btn btn-secondary btn-xs" name="add_dokumentasi_inspeksiapar_v2" value="Upload"><br><br>
                          </form>
                      </td>
                    <tr>
